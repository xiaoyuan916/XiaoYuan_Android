...
public class AutoScannerView extends View {
    ...
    private final int triAngleColor = Color.parseColor("#009d85");                        //边角的颜色
    private final int lineColor = Color.parseColor("#009d85");                            //中间线的颜色
...
//            canvas.drawLine(frame.left, frame.top + lineOffsetCount, frame.right, frame.top + lineOffsetCount, linePaint);    //画一条线
            Rect lineRect = new Rect();
            lineRect.left = frame.left;
            lineRect.top = frame.top + lineOffsetCount;
            lineRect.right = frame.right;
            lineRect.bottom = frame.top + dp2px(10) + lineOffsetCount;
            canvas.drawBitmap(((BitmapDrawable)(getResources().getDrawable(R.drawable.scanline_blue))).getBitmap(), null, lineRect, linePaint);
        }
