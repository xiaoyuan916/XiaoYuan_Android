...
abstract public class PmPacket {
	...
	private byte afn;
	private final PmPacketData dataBuff;
	private final EventCountor eventCountor;
	private final Authorize authorize;
	private final TimeProtectValue tpv;
	...
	public PmPacket setAuthorize(Authorize authorize) {
		this.authorize.setValue(authorize.getValue());
	...
	public PmPacket setTpv(TimeProtectValue tpv) {
		this.tpv.setValue(tpv.getValue());
		return this;
	}
	...
	public byte[] getValue() {
		...
		} else if ((controlCode.getIsUpDirect())
				&& (controlCode.getUpDirectIsAppealCall())) {
			len = len + 2;
		}
	...
	public PmPacket setValue(byte[] msg, int firstIndex) {
		...
		if (head != -1) {
			...
			int len = (BcdUtils.byteToUnsigned(msg[head + 1]) + BcdUtils
					.byteToUnsigned(msg[head + 2]) * 0x100) >> 2;
	...
	protected static int getMsgHeadOffset(byte[] msg, byte protocolVersion,
		...
		while (head != -1) {
			int len = (BcdUtils.byteToUnsigned(msg[head + 1]) + BcdUtils
					.byteToUnsigned(msg[head + 2]) * 0x0100) >> 2;

//			if ((msg.length >= (head + len + 7))
			if ((msg.length > (head + len + 7))
					&& (msg[head + len + 7] == 0x16)) {
				if (PmPacket.calcCs(msg, head + 6, head + len + 6) == msg[head
						+ len + 6]) {
					return head;
				}
			}
	...
	private static int getHeadOffset(byte[] msg, int beginIndex,
		...
		for (int i = beginIndex; i < msg.length
				- (1 + 2 + 2 + 1 + 1 + 5 + 1 + 1 + 1 + 1); i++) {// 68 L L L L
																	// 68 C R R
																	// R R R A S
																	// C 16 最短
			if ((msg[i] == 0x68) && (msg[i + 1] == msg[i + 3])
					&& (msg[i + 2] == msg[i + 4]) && (msg[i + 5] == 0x68)
					&& ((msg[i + 1] & 0x03) == protocolVersion)) {
				head = i;
				break;
			}
		}
