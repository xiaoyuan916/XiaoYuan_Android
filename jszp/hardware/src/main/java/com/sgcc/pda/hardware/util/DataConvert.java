...
//            sb.append(" ");
    ...
    /**
     * 获取一段字符串的长度的HEX表示
     * 例如输入“010203040506”,1返回“06”，输入“010203040506”，2返回“0006”
     * @param value 待计算字符串
    ...
    /**
     * 改变字符串长度
     * 若字符串实际长度大于length，则返回s，若小于则前面补0
     * @param s
     * @param length
     * @return
     */
    public static String changeStringLength(String s, int length) {
        if (!TextUtils.isEmpty(s) && s.length() < length) {
            StringBuffer sb = new StringBuffer();
            for (int i = 0, j = length - s.length(); i < j; i++) {
                sb.append(0);
            }
            sb.append(s);
            s = sb.toString();
