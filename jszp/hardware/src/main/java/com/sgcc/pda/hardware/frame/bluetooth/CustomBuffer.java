...
public class CustomBuffer {
    private static final String TAG = "CustomBuffer";

    // 判断整个报文是否完整的标识
//    private final static byte MSG_START_FLAG = (byte) 0XC9;
//    private final static byte MSG_STOP_FLAG = (byte) 0X9C;
    private final static byte MSG_START_FLAG = (byte) 0X97;
    private final static byte MSG_STOP_FLAG = (byte) 0XE9;
     ...
     *
     * @param out_data 读取出来的整个报文
     * @param out_len  报文的字节数
     * @return
     */
    public int GetCompleteMessage(byte[] out_data, int[] out_len) {
        ...
        for (int start = 0; start < BUFFER_SIZE; start++) {
            if (mBuffer[start] == MSG_START_FLAG && mBuffer[start + 5] == MSG_START_FLAG) { // 起始符和标识符正确
                ...
                int data_filed_len = ByteUtils.byteToInt(temp_buf, 0, 2);// 数据域长度

                Log.i(TAG, "=======data_filed_len ======" + data_filed_len);
    ...
    public static boolean isFrameOk(byte[] datas) {
        boolean flag = false;
        ...
        try {
            for (int start = 0; start < datas.length; start++) {
                if (datas[start] == MSG_START_FLAG && datas[start + 5] == MSG_START_FLAG) { // 起始符和标识符正确

                    start_index = start;

                    byte[] temp_buf = new byte[2];
                    temp_buf[0] = datas[start_index + 3];
                    temp_buf[1] = datas[start_index + 4];
                    int data_filed_len = ByteUtils.byteToInt(temp_buf, 0, 2);// 数据域长度

                    len = 8 + data_filed_len;

                    stop_index = start + len - 1;

                    if (datas[stop_index] == MSG_STOP_FLAG) {// 结束符正确

                        byte check_code = datas[stop_index - 1];
                        byte temp_code = 0;
                        for (int i = start_index; i <= stop_index - 2; i++) {
                            temp_code = (byte) (temp_code + datas[i]);
                        }
                        if (temp_code == check_code) {// 检验码正确
                            // 是正常完整报文，返回
                            flag = true;
     ...
     * @return
     */
    public int getDataMessage(byte[] out_data, int[] out_len) {
        int flag = -1;
        ...
        for (int start = 0; start < BUFFER_SIZE; start++) {
            if (mBuffer[start] == MSG_START_FLAG && mBuffer[start + 11] == MSG_START_FLAG) { // 起始符和标识符正确

                start_index = start;
                byte[] temp_buf = new byte[2];
                temp_buf[0] = mBuffer[start_index + 12];
                temp_buf[1] = mBuffer[start_index + 13];
                int data_filed_len = DataConvert.byteToInt(temp_buf, 0, 2);// 数据域长度

                Log.i(TAG, "=======data_filed_len ======" + data_filed_len);

                out_len[0] = 20 + data_filed_len;
