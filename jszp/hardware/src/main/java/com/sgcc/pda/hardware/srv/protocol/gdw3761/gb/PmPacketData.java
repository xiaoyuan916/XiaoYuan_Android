...
public class PmPacketData extends BcdDataBuffer {
	...
	public String getAscii(int len) {
		try {
			String str = dataBuff.getString(len, Charset.forName("US-ASCII")
					.newDecoder());
			return str;
		} catch (CharacterCodingException ex) {
			Logger.getLogger(PmPacketData.class.getName()).log(Level.SEVERE,
					null, ex);
		}
		return "";
	}
	...
	public String getBS8() {
		...
		dataBuff.get(bytes);
		return BcdUtils.byteToBitSetString(bytes);
	}

	public PmPacketData putBS16(String bs8) {
		dataBuff.put(BcdUtils.bitSetStringToBytes(bs8));
		return this;
	}
