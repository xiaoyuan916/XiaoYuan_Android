...
public class Parser {
    ...
    private Frame getframe(byte[] reciveBytes) {
        ...
        Log.e("tag", "----len-------" + DataConvert.toHexString(reciveBytes, 2, 2));
        frame.setDataLen(Integer.valueOf(DataConvert.toHexString(reciveBytes, 2, 2), 16)); //帧长度

        byte sw1 = (byte) (reciveBytes[0] & 0xFF);
        frame.setSw1(sw1);//状态字1

        byte sw2 = (byte) (reciveBytes[1] & 0xFF);
        frame.setSw1(sw2);//状态字2

        byte[] temp = new byte[frame.getDataLen()];
        System.arraycopy(reciveBytes, 4, temp, 0, frame.getDataLen());
