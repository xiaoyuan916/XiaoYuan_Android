package com.sgcc.pda.hardware.srv.protocol.dl645;
...
import com.sgcc.pda.hardware.srv.protocol.gdw3761.gb.utils.buffer.IoBuffer;
...
public class Gb645MeterPacket {
	private final Gb645Address address;
	private final Gb645ControlCode controlCode;
	private final BcdDataBuffer data; // 总是保存未进行任何加33减33处理的值
	...
	public Gb645MeterPacket setControlCode(boolean isFromMast,
			boolean hasHouxuzhen, boolean isYichang, byte gongnengma) {
		controlCode.isFromMast(isFromMast).isHasHouxuzhen(hasHouxuzhen)
				.isYichang(isYichang).setFuncCode(gongnengma);
	...
	public byte[] getValue() {
		...
		IoBuffer bf = IoBuffer.allocate(80, false);
	...
	private static int getHeadOffset(byte[] msg, int beginIndex) {
		...
		for (int i = beginIndex; i < msg.length - 11; i++) {// 68 R R R R R R 68
															// C L C 16 最短
	...
	public static int getMsgHeadOffset(byte[] msg, int firstIndex) {
		...
		while (head != -1) {
			...
			if (msg[head + len + 11] == 0x16) {
				if (Gb645MeterPacket.calcCs(msg, head, len + 10) == msg[head
						+ len + 10]) {
					return head;
				}
			}
