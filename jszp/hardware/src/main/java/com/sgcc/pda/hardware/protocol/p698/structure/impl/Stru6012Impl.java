...
//   批量设置、操作方法批量增加或更新 任务配置单元
public class Stru6012Impl  extends IStructure {
	@Override
	public  byte[]   getBytes(){
		List<Object> listTask =  this.getObj();
		int  pos=0;
		byte  []  temp = new byte[1024];
		byte  []  value; 
		DataItem698 di  =  new DataItem698();
		temp[pos++] = CommUtils.hex2Binary(Integer.toHexString(listTask.size()))[0];    //array包含对象个数
		for(Object obj :  listTask ){
				TaskConfUnit tcu= (TaskConfUnit)obj;
