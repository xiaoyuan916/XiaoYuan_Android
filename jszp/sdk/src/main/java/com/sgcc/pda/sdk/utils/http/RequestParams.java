...
public class RequestParams implements Serializable {
    ...
    public MultipartBody.Builder createMultipartBody() {
        ...
        for (ConcurrentHashMap.Entry<String, FileWrapper> entry : fileParams.entrySet()) {
            ...
            //不带参数的文件上传
            RequestBody body = RequestBody.create(TextUtils.isEmpty(fileWrapper.contentType) ? null : MediaType.parse(fileWrapper.contentType), fileWrapper.file);
        ...
        for (ConcurrentHashMap.Entry<String, List<FileWrapper>> entry : fileArrayParams.entrySet()) {
            ...
            for (FileWrapper fw : fileWrapper) {
                //不带参数的文件上传
                RequestBody body = RequestBody.create(TextUtils.isEmpty(fw.contentType) ? null : MediaType.parse(fw.contentType), fw.file);
                multipartBuilder.addPart(body);
                //带参数的文件上传
